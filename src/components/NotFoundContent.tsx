...
/**
 * Содержимое страницы 404. Вынесено в компонент, потому что рендерится из двух мест:
 *  - app/(ru)/not-found.tsx — 404 внутри русской группы (оболочку даёт её root layout);
 *  - app/not-found.tsx — глобальная 404 для адресов, не попавших ни в одну группу.
 *    Корневого layout в app/ больше нет (их два, по одному на язык), поэтому
 *    глобальная 404 обязана отрисовать оболочку сама — иначе отдаётся голый HTML
 *    без шапки, подвала и стилей.
 */
const popularLinks = [
